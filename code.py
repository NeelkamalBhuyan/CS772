...
class NeuralNet:
    ...
    def build_nn(self):
        ...
        model = keras.Sequential(
    		[
        		layers.Dense(5, input_shape = (10, self.reviews.shape[1]), activation='softmax'),
    		]
		)
        

    def train_nn(self,batch_size,epochs_train):
        # write the training loop here; you can use either tensorflow or pytorch
        # print validation accuracy
        loss_fn = tf.keras.losses.CategoricalCrossentropy()
        model.compile(optimizer='adam', loss=loss_fn, metrics=['accuracy'])
        x_train = self.reviews[0:len(self.reviews)*5/6]
        y_train = self.ratings[0:len(self.ratings)*5/6]
        y_train = tf.one_hot(y_train, 5)

        x_val = self.reviews[len(self.reviews)*5/6:]
        y_val = self.ratings[len(self.ratings)*5/6:]
        y_val = tf.one_hot(y_val, 5)

        model.fit(x_train, y_train, epochs=epochs_train)
        model.evaluate(x_val,  y_val, verbose=2)

    def predict(self, reviews):
    	pred = model(reviews).numpy()
    	prediction = np.argmax (pred, axis=1) + 1
    	return prediction
...
def main(train_file, test_file):
    ...
    model.build_nn()
    model.train_nn(batch_size,epochs_train)
